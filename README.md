...
- `-DBACKLIGHT_IGNORE_HOST=ON` — ignore all host (kernel/Android) writes to the backlight register. Backlight is controlled only by the firmware key combo (toggle on/off), and optionally persisted if `BACKLIGHT_PERSIST=ON`. Use this to avoid "backlight auto-on on unlock" until the kernel is updated. The combo is: **Sym or Alt (Right Alt) + the key that has 0 on it** (firmware sees that key as `~` or `0` depending on modifiers). If your kernel maps Sym to AltRight and that key to é, the same physical keys still trigger the combo.
